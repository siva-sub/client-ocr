 ...
 * limitations under the License.
 * =============================================================================
 */function Qh(e){return e.includes("ppu-paddle-ocr")||e.includes("PP-OCRv5_mobile_det")||e.includes("PP-OCRv4_mobile_rec")}function Yh(e){const t=e.split(`
...
`).filter(n=>n.length>0),r.dictionary||(r.dictionary=r.character)))}catch(i){console.warn("Failed to extract metadata from model:",i)}return r}static hasEmbeddedDictionary(t){return!!(t.metadata.dictionary&&t.metadata.dictionary.length>0)}static async getDictionary(t,r){if(this.hasEmbeddedDictionary(t))return console.log("Using embedded dictionary from model metadata"),t.metadata.dictionary;if(r){console.log("Loading external dictionary from:",r);try{const a=await(await fetch(r)).text();return Qh(r)?Yh(a):a.split(`
`).filter(n=>n.length>0)}catch(i){throw console.error("Failed to load external dictionary:",i),i}}throw new Error("No dictionary available: model has no embedded dictionary and no fallback path provided")}}let Rt=null,Te=null;self.addEventListener("message",async e=>{const{type:t,data:r}=e.data;switch(t){case"INIT":try{console.log("Initializing detection model:",r.modelPath),Rt=await Jh.loadModel(r.modelPath),console.log("Detection model initialized successfully"),console.log("Input names:",Rt.session.inputNames),console.log("Output names:",Rt.session.outputNames),Te=r.config?.det||{engine_type:"onnxruntime",lang_type:"en",model_type:"mobile",ocr_version:"PP-OCRv4",limit_side_len:736,limit_type:"min",std:[.5,.5,.5],mean:[.5,.5,.5],scale:1/255,thresh:.3,box_thresh:.5,max_candidates:1e3,unclip_ratio:1.6,use_dilation:!0,score_mode:"fast"},self.postMessage({type:"RESULT",data:{initialized:!0}})}catch(i){self.postMessage({type:"ERROR",error:i.message})}break;case"DETECT":if(!Rt||!Te){self.postMessage({type:"ERROR",error:"Model not initialized"});return}try{const{imageData:i,width:a,height:n}=r,{resizedData:s,resizedWidth:u,resizedHeight:l,ratioH:d,ratioW:c}=em(i,a,n),f=tm(s,u,l),_={[Rt.session.inputNames[0]||"input"]:f},g=await Rt.session.run(_),b=rm(g,u,l,d,c);console.log(`Detection found ${b.length} text boxes`),b.length>0&&console.log("First box:",b[0]),self.postMessage({type:"RESULT",data:{boxes:b}})}catch(i){self.postMessage({type:"ERROR",error:i.message})}break}});function em(e,t,r){if(!Te)throw new Error("Config not initialized");const i=Te.limit_side_len,a=Te.limit_type;let n=1;a==="max"?Math.max(r,t)>i&&(n=r>t?i/r:i/t):Math.min(r,t)<i&&(n=r<t?i/r:i/t);let s=Math.round(r*n),u=Math.round(t*n);s=Math.ceil(s/32)*32,u=Math.ceil(u/32)*32,s=Math.max(s,32),u=Math.max(u,32),console.log(`Detection resize: ${t}x${r} → ${u}x${s} (ratio: ${n.toFixed(3)})`);const d=new OffscreenCanvas(u,s).getContext("2d"),c=new ImageData(new Uint8ClampedArray(e),t,r),f=new OffscreenCanvas(t,r);return f.getContext("2d").putImageData(c,0,0),d.drawImage(f,0,0,t,r,0,0,u,s),{resizedData:d.getImageData(0,0,u,s).data,resizedWidth:u,resizedHeight:s,ratioH:s/r,ratioW:u/t}}function tm(e,t,r){if(!Te)throw new Error("Config not initialized");const i=3,a=new Float32Array(i*r*t),n=Te.scale||1/255,s=Te.mean||[.5,.5,.5],u=Te.std||[.5,.5,.5];console.log(`Detection preprocessing: using scale=${n}, mean=${s}, std=${u}`);for(let l=0;l<r;l++)for(let d=0;d<t;d++){const c=(l*t+d)*4,f=l*t+d;a[f]=(e[c]*n-s[0])/u[0],a[r*t+f]=(e[c+1]*n-s[1])/u[1],a[2*r*t+f]=(e[c+2]*n-s[2])/u[2]}return new Ue("float32",a,[1,i,r,t])}function rm(e,t,r,i,a){if(!Te)throw new Error("Config not initialized");const n=Object.keys(e)[0],s=e[n],u=s.data,l=s.dims;console.log("Detection output shape:",l),console.log("Detection raw output sample:",u.slice(0,10));let d,c;if(l.length===4){const[g,b,k,v]=l;d=k,c=v}else if(l.length===3){const[g,b,k]=l;d=b,c=k}else return console.error("Unexpected detection output shape:",l),[];const f=new Uint8Array(d*c);for(let g=0;g<d;g++)for(let b=0;b<c;b++){const k=g*c+b,v=u[k];f[k]=v>Te.thresh?255:0}const h=f.filter(g=>g===255).length;return console.log(`Detection bitmap: ${h} pixels above threshold (${(h/f.length*100).toFixed(2)}%)`),Te.use_dilation&&im(f,c,d),am(f,c,d,t,r).map(g=>({topLeft:{x:Math.round(g.topLeft.x/a),y:Math.round(g.topLeft.y/i)},topRight:{x:Math.round(g.topRight.x/a),y:Math.round(g.topRight.y/i)},bottomRight:{x:Math.round(g.bottomRight.x/a),y:Math.round(g.bottomRight.y/i)},bottomLeft:{x:Math.round(g.bottomLeft.x/a),y:Math.round(g.bottomLeft.y/i)}}))}function im(e,t,r){const i=[[1,1],[1,1]],a=i.length,n=i[0].length,s=new Uint8Array(e);for(let u=0;u<r;u++)for(let l=0;l<t;l++){const d=u*t+l;let c=!1;for(let f=0;f<a;f++){for(let h=0;h<n;h++){const _=u+f-Math.floor(a/2),g=l+h-Math.floor(n/2);if(_>=0&&_<r&&g>=0&&g<t){const b=_*t+g;if(s[b]===255&&i[f][h]===1){c=!0;break}}}if(c)break}e[d]=c?255:0}}function am(e,t,r,i,a){if(!Te)throw new Error("Config not initialized");const n=new Uint8Array(e.length),s=[];let u=0,l=0;const d=Te.minimum_area_threshold||20,c=Te.padding_vertical||.4,f=Te.padding_horizontal||.6;for(let h=0;h<r;h++)for(let _=0;_<t;_++){const g=h*t+_;if(e[g]===255&&!n[g]){const b=nm(e,n,_,h,t,r);if(u++,(b.maxX-b.minX)*(b.maxY-b.minY)>d){l++;const v=b.maxY-b.minY,w=Math.round(v*c),S=Math.round(v*f);let x=b.minX-S,T=b.maxX+S,z=b.minY-w,E=b.maxY+w;x=Math.max(0,x),T=Math.min(t-1,T),z=Math.max(0,z),E=Math.min(r-1,E);const O=i/t,N=a/r,U={topLeft:{x:Math.round(x*O),y:Math.round(z*N)},topRight:{x:Math.round(T*O),y:Math.round(z*N)},bottomRight:{x:Math.round(T*O),y:Math.round(E*N)},bottomLeft:{x:Math.round(x*O),y:Math.round(E*N)}};if(sm(e,b,t)>Te.box_thresh){const H=U.topRight.x-U.topLeft.x,ee=U.bottomLeft.y-U.topLeft.y;H>5&&ee>5&&s.push(U)}}}}return console.log(`Connected components: found=${u}, aboveMinSize=${l}, finalBoxes=${s.length}`),s}function nm(e,t,r,i,a,n){const s={minX:r,maxX:r,minY:i,maxY:i,pixels:0},u=[[r,i]];for(;u.length>0;){const[l,d]=u.pop(),c=d*a+l;l<0||l>=a||d<0||d>=n||t[c]||e[c]!==255||(t[c]=1,s.pixels++,s.minX=Math.min(s.minX,l),s.maxX=Math.max(s.maxX,l),s.minY=Math.min(s.minY,d),s.maxY=Math.max(s.maxY,d),u.push([l+1,d],[l-1,d],[l,d+1],[l,d-1]))}return s}function sm(e,t,r){let i=0,a=0;for(let n=t.minY;n<=t.maxY;n++)for(let s=t.minX;s<=t.maxX;s++)i+=e[n*r+s]/255,a++;return a>0?i/a:0}})();
