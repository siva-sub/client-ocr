...
var best_index : i32 = 0;`,`if (${i.getByIndices("input_indices")} ${t.selectLastIndex>0?">=":">"} value) {
  ...
  }`}},$o=e=>{bo(e.inputs),e.compute(wo(e.inputs))}}),vo,de,xo,ko,So,To,Io,Eo,zo,Co,Oo,Ao,Bo,Ro,Mo,No,Vt,Do,br,Po,Uo,qo,Wo,Lo,Vo,Go,Ho,Fo,jo,Ko,Zo,Qo,Xo,Yo,Jo,Mi,eu,Ni,Di,tu,ru,iu,au,nu,su,Pi=P(()=>{J(),re(),we(),ne(),vo=(e,t,r,i,a,n,s)=>{let u=Math.ceil(t/4),l="";typeof a=="string"?l=`${a}(a)`:l=a("a");let d=B("inputData",r,[u],4),c=G("outputData",i,[u],4),f=[{name:"vec_size",type:"u32"}];return s&&f.push(...s),`
...
}`,Po=e=>{let t=Ie(e.inputs[0].dataType);e.compute(de(e.inputs[0],"Erf",r=>`erf_vf32(${r})`,br(t)))},Uo=e=>{e.compute(de(e.inputs[0],"Exp","exp"))},qo=e=>{e.compute(de(e.inputs[0],"Floor","floor"))},Wo=e=>{let t=Ie(e.inputs[0].dataType);e.compute(de(e.inputs[0],"Gelu",r=>`0.5 * ${r} * (1.0 + erf_vf32(${r} * 0.7071067811865475))`,br(t)))},Lo=(e,t)=>{let r=Ie(e.inputs[0].dataType);e.compute(de(e.inputs[0],"LeakyRelu",i=>`select(leaky_relu_alpha_ * ${i}, ${i}, ${i} >= vec4<${r}>(0.0))`,`const leaky_relu_alpha_ = ${r}(${t.alpha});`,t.cacheKey))},Vo=e=>{e.compute(de(e.inputs[0],"Not",t=>`!${t}`))},Go=e=>{e.compute(de(e.inputs[0],"Neg",t=>`-${t}`))},Ho=e=>{e.compute(de(e.inputs[0],"Reciprocal",t=>`1.0/${t}`))},Fo=e=>{let t=Ie(e.inputs[0].dataType);e.compute(de(e.inputs[0],"Relu",r=>`select(vec4<${t}>(0.0), ${r}, ${r} > vec4<${t}>(0.0))`))},jo=e=>{e.compute(de(e.inputs[0],"Sigmoid",t=>`(1.0 / (1.0 + exp(-${t})))`))},Ko=e=>ce(e),Zo=(e,t)=>{let r=Ie(e.inputs[0].dataType);e.compute(de(e.inputs[0],"HardSigmoid",i=>`max(vec4<${r}>(0.0), min(vec4<${r}>(1.0), ${t.alpha} * ${i} + vec4<${r}>(${t.beta})))`,void 0,t.cacheKey))},Qo=e=>{e.compute(de(e.inputs[0],"Sin","sin"))},Xo=e=>{e.compute(de(e.inputs[0],"Sinh","sinh"))},Yo=e=>{e.compute(de(e.inputs[0],"Sqrt","sqrt"))},Jo=e=>{e.compute(de(e.inputs[0],"Tan","tan"))},Mi=e=>`sign(${e}) * (1 - exp(-2 * abs(${e}))) / (1 + exp(-2 * abs(${e})))`,eu=e=>{e.compute(de(e.inputs[0],"Tanh",Mi))},Ni=(e="f32")=>`
...
`}),Gt,Wi,Li=P(()=>{J(),re(),ne(),$t(),Gt=(e,t,r,i,a)=>{let n=i-r;return`
...
`},Wi=(e,t,r,i,a=!1,n)=>{let s=e[0].dims,u=e[1].dims,l=s[s.length-2],d=u[u.length-1],c=s[s.length-1],f=be(d),h=be(c),g=be(l),_=C.size(r)/f/g,b=e.length>2,k=i?i.slice(0,-2):r.slice(0,-2),v=[C.size(k),l,d],w=[{type:12,data:_},{type:12,data:l},{type:12,data:d},{type:12,data:c}];bt(t,w),w.push(...Z(k,s,u)),b&&w.push(...Z(e[2].dims)),w.push(...Z(v));let S=x=>{let T=Ei("batch_dims",e[0].dataType,k.length),z=B("a",e[0].dataType,s.length,h),E=B("b",e[1].dataType,u.length,f),A=G("output",e[0].dataType,v.length,f),N=ke(A.type.tensor),L=yt(t,A.type.value,N),Y=[z,E],F="";if(b){let ie=a?f:1;Y.push(B("bias",e[2].dataType,e[2].dims.length,ie)),F=`${a?`value += bias[col / ${ie}];`:`value += ${A.type.value}(bias[row + i]);`}`}let ee=[{name:"output_size",type:"u32"},{name:"M",type:"u32"},{name:"N",type:"u32"},{name:"K",type:"u32"}];wt(t,ee);let U=()=>{let ie=`var a_data: ${z.type.value};`;for(let Q=0;Q<h;Q++)ie+=`
...
`}return ie};return`
  ...
  `};return{name:"MatMulNaive",shaderCache:{hint:`${t.activation};${f};${h};${g};${a}`,inputDependencies:b?["rank","rank","rank"]:["rank","rank"]},getRunData:()=>({outputs:[{dims:n?n(r):r,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(_/64)},programUniforms:w}),getShaderSource:S}}}),Cu,Ou,Vi,Gi,Au,Hi,Bu,wr,Fi=P(()=>{J(),re(),ne(),$t(),Li(),qi(),Cu=(e,t)=>e?`
