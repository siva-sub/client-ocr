...
}`,Po=e=>{let t=Ie(e.inputs[0].dataType);e.compute(de(e.inputs[0],"Erf",r=>`erf_vf32(${r})`,wr(t)))},Uo=e=>{e.compute(de(e.inputs[0],"Exp","exp"))},qo=e=>{e.compute(de(e.inputs[0],"Floor","floor"))},Wo=e=>{let t=Ie(e.inputs[0].dataType);e.compute(de(e.inputs[0],"Gelu",r=>`0.5 * ${r} * (1.0 + erf_vf32(${r} * 0.7071067811865475))`,wr(t)))},Lo=(e,t)=>{let r=Ie(e.inputs[0].dataType);e.compute(de(e.inputs[0],"LeakyRelu",i=>`select(leaky_relu_alpha_ * ${i}, ${i}, ${i} >= vec4<${r}>(0.0))`,`const leaky_relu_alpha_ = ${r}(${t.alpha});`,t.cacheKey))},Vo=e=>{e.compute(de(e.inputs[0],"Not",t=>`!${t}`))},Go=e=>{e.compute(de(e.inputs[0],"Neg",t=>`-${t}`))},Ho=e=>{e.compute(de(e.inputs[0],"Reciprocal",t=>`1.0/${t}`))},Fo=e=>{let t=Ie(e.inputs[0].dataType);e.compute(de(e.inputs[0],"Relu",r=>`select(vec4<${t}>(0.0), ${r}, ${r} > vec4<${t}>(0.0))`))},jo=e=>{e.compute(de(e.inputs[0],"Sigmoid",t=>`(1.0 / (1.0 + exp(-${t})))`))},Ko=e=>ce(e),Zo=(e,t)=>{let r=Ie(e.inputs[0].dataType);e.compute(de(e.inputs[0],"HardSigmoid",i=>`max(vec4<${r}>(0.0), min(vec4<${r}>(1.0), ${t.alpha} * ${i} + vec4<${r}>(${t.beta})))`,void 0,t.cacheKey))},Xo=e=>{e.compute(de(e.inputs[0],"Sin","sin"))},Qo=e=>{e.compute(de(e.inputs[0],"Sinh","sinh"))},Yo=e=>{e.compute(de(e.inputs[0],"Sqrt","sqrt"))},Jo=e=>{e.compute(de(e.inputs[0],"Tan","tan"))},Mi=e=>`sign(${e}) * (1 - exp(-2 * abs(${e}))) / (1 + exp(-2 * abs(${e})))`,eu=e=>{e.compute(de(e.inputs[0],"Tanh",Mi))},Ni=(e="f32")=>`
...
`,nu=e=>`quick_gelu_impl(${e})`,su=(e,t)=>{let r=Ie(e.inputs[0].dataType);e.compute(de(e.inputs[0],"QuickGelu",nu,au(r,t.alpha),t.cacheKey,e.inputs[0].dataType))}}),ou,uu,lu,oh=P(()=>{re(),ne(),Pi(),ou=e=>{if(e[0].dims.length!==3)throw new Error("input should have 3 dimensions");if(![2560,5120,10240].includes(e[0].dims[2]))throw new Error("hidden state should be 2560, 5120 or 10240");if(e[1].dims.length!==1)throw new Error("bias is expected to have 1 dimensions");if(e[0].dims[2]!==e[1].dims[0])throw new Error("last dimension of input and bias are not the same")},uu=e=>{let t=e[0].dims.slice();t[2]=t[2]/2;let r=B("input",e[0].dataType,e[0].dims,4),i=B("bias",e[0].dataType,[e[0].dims[2]],4),a=G("output",e[0].dataType,t,4),n=C.size(t)/4,s=ke(e[0].dataType);return{name:"BiasSplitGelu",getRunData:()=>({outputs:[{dims:t,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(n/64)}}),getShaderSource:u=>`
  ...
  ${u.mainStart()}
    ...
    ${a.setByOffset("global_idx","valueLeft * geluRight")}
        ...
        ${e.guardAgainstOutOfBoundsWorkgroupSizes("uniforms.vec_size")}
        ${v}
      }`},pu=(e,t,r,i,a,n,s=r.dataType)=>{let u=r.dims.map(b=>Number(b)??1),l=i.dims.map(b=>Number(b)??1),d=!C.areEqual(u,l),c=u,f=C.size(u),h=!1,g=!1,_=[d];if(d){let b=Ct.calcShape(u,l,!1);if(!b)throw new Error("Can't perform binary op on the given tensors");c=b.slice(),f=C.size(c);let k=C.size(u)===1,v=C.size(l)===1,w=u.length>0&&u[u.length-1]%4===0,S=l.length>0&&l[l.length-1]%4===0;_.push(k),_.push(v),_.push(w),_.push(S);let x=1;for(let T=1;T<c.length;T++){let z=u[u.length-T],E=l[l.length-T];if(z===E)x*=z;else break}x%4===0?(g=!0,h=!0):(k||v||w||S)&&(h=!0)}else h=!0;return _.push(h),{name:e,shaderCache:{hint:t+_.map(b=>b.toString()).join("_"),inputDependencies:["rank","rank"]},getShaderSource:b=>du(b,u,l,c,h,d,g,a,r.dataType,i.dataType,s,n),getRunData:()=>({outputs:[{dims:c,dataType:s}],dispatchGroup:{x:Math.ceil(f/64/4)},programUniforms:[{type:12,data:Math.ceil(C.size(c)/4)},...Z(u,l,c)]})}},je=(e,t,r,i,a,n)=>{e.compute(pu(t,a??"",e.inputs[0],e.inputs[1],r,i,n))},cu=e=>{je(e,"Add",(t,r)=>`${t}+${r}`)},fu=e=>{je(e,"Div",(t,r)=>`${t}/${r}`)},hu=e=>{je(e,"Equal",{scalar:(t,r)=>`u32(${t}==${r})`,vector:(t,r)=>`vec4<u32>(${t}==${r})`},void 0,void 0,9)},mu=e=>{je(e,"Mul",(t,r)=>`${t}*${r}`)},gu=e=>{let t=B("input",e.inputs[0].dataType,e.inputs[0].dims).type.value;je(e,"Pow",{scalar:(r,i)=>`pow_custom(${r},${i})`,vector:(r,i)=>`pow_vector_custom(${r},${i})`},`
...
`},Wi=(e,t,r,i,a=!1,n)=>{let s=e[0].dims,u=e[1].dims,l=s[s.length-2],d=u[u.length-1],c=s[s.length-1],f=be(d),h=be(c),g=be(l),_=C.size(r)/f/g,b=e.length>2,k=i?i.slice(0,-2):r.slice(0,-2),v=[C.size(k),l,d],w=[{type:12,data:_},{type:12,data:l},{type:12,data:d},{type:12,data:c}];wt(t,w),w.push(...Z(k,s,u)),b&&w.push(...Z(e[2].dims)),w.push(...Z(v));let S=x=>{let T=Ei("batch_dims",e[0].dataType,k.length),z=B("a",e[0].dataType,s.length,h),E=B("b",e[1].dataType,u.length,f),A=G("output",e[0].dataType,v.length,f),N=ke(A.type.tensor),L=bt(t,A.type.value,N),Y=[z,E],F="";if(b){let ie=a?f:1;Y.push(B("bias",e[2].dataType,e[2].dims.length,ie)),F=`${a?`value += bias[col / ${ie}];`:`value += ${A.type.value}(bias[row + i]);`}`}let ee=[{name:"output_size",type:"u32"},{name:"M",type:"u32"},{name:"N",type:"u32"},{name:"K",type:"u32"}];$t(t,ee);let U=()=>{let ie=`var a_data: ${z.type.value};`;for(let X=0;X<h;X++)ie+=`
...
`},Bu=(e,t,r,i,a=!1)=>{let[n,s,u,l]=i,d=ke(i[0].type.tensor);return`
  ...
  }`};return{name:"GroupedConv",shaderCache:{hint:`${t.cacheKey}_${f}`,inputDependencies:_},getRunData:()=>({outputs:[{dims:i?i(r):r,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(h/64)},programUniforms:g}),getShaderSource:b}},Lu=(e,t,r,i)=>{let a=e.length>2,n=be(r[3]),s=be(r[2]),u=C.size(r)/n/s,l=[e[0].dims[0],e[0].dims[1],e[0].dims[2],e[0].dims[3]/n],d=[e[1].dims[0],e[1].dims[1],e[1].dims[2],e[1].dims[3]/n],c=[r[0],r[1],r[2],r[3]/n],f=[{type:12,data:u},{type:6,data:[t.strides[0],t.strides[1]]},{type:6,data:[t.pads[0],t.pads[1]]}];wt(t,f),f.push(...Z(l,d,c));let h=(s-1)*t.strides[1]+d[1],g=_=>{let b=G("output",e[0].dataType,c.length,n),k=ke(b.type.tensor),v=bt(t,b.type.value,k),w=B("x",e[0].dataType,l.length,n),S=B("w",e[1].dataType,d.length,n),x=[w,S];a&&x.push(B("b",e[2].dataType,e[2].dims,n));let T=a?"value += b[output_channel];":"",z=[{name:"output_size",type:"u32"},{name:"strides",type:"i32",length:2},{name:"pads",type:"i32",length:2}];return $t(t,z),`
...
`)};
  ...
  ${h.mainStart()}
    ...
    }`};return{name:"GatherBlockQuantized",shaderCache:{hint:`${t.cacheKey};${e.filter((g,_)=>_!==1).map(g=>g.dims.join("_")).join(";")}`,inputDependencies:Array.from({length:e.length},(g,_)=>"rank")},getRunData:()=>({outputs:[{dims:u,dataType:d}],dispatchGroup:{x:Math.ceil(l/64)},programUniforms:f}),getShaderSource:h}},Ol=(e,t)=>{let r=e.inputs;zl(r,t),e.compute(Cl(e.inputs,t))},Al=e=>ce({blockSize:e.blockSize,gatherAxis:e.gatherAxis,quantizeAxis:e.quantizeAxis})}),Bl,Rl,Ml,Nl,Sh=P(()=>{J(),re(),we(),ne(),Bl=e=>{if(!e||e.length!==2)throw new Error("GatherElements requires 2 inputs.");if(e[0].dims.length<1)throw new Error("GatherElements requires that the data input be rank >= 1.");if(e[0].dims.length!==e[1].dims.length)throw new Error(`GatherElements requires that the data input and
...
`)}
  ...
  }`};e.compute({name:"InstanceNormalization",shaderCache:{hint:`${d}`,inputDependencies:_},getRunData:()=>({outputs:[{dims:a,dataType:t[0].dataType}],dispatchGroup:{x:Math.ceil(c/64)},programUniforms:[{type:12,data:c},...Z(h,g,h)]}),getShaderSource:b},{inputs:[t[0],f]})},_d=(e,t,r)=>{let i=t[0].dims,a=i,n=i[0],s=i[i.length-1],u=C.sizeFromDimension(i,1)/s,l=be(s),d=C.size(a)/l,c=[{type:12,data:u},{type:12,data:Math.floor(s/l)}],f=["type","type"],h=!1,g=[0,i.length-1];for(let v=0;v<i.length-2;v++)h=h||i[v+1]!==1,g.push(v+1);h=h&&i[i.length-1]!==1;let _=h?e.compute(Re(e.inputs[0],g),{inputs:[e.inputs[0]],outputs:[-1]})[0]:e.inputs[0].reshape(Array.from({length:i.length},(v,w)=>i[g[w]])),b=sa(e,_,t[1],t[2],n,u,s,r.epsilon),k=v=>{let w=ke(t[0].dataType),S=l===1?"vec2f":`mat${l}x2f`,x=E=>{let A=E===0?"x":"y",N=l===1?"f32":`vec${l}f`;switch(l){case 1:return`${w}(${N}(scale.${A}))`;case 2:return`vec2<${w}>(${N}(scale[0].${A}, scale[1].${A}))`;case 4:return`vec4<${w}>(${N}(scale[0].${A}, scale[1].${A}, scale[2].${A}, scale[3].${A}))`;default:throw new Error(`Not supported compoents ${l}`)}},T=B("input",t[0].dataType,t[0].dims,l),z=G("output",t[0].dataType,a,l);return`
...
`).filter(n=>n.length>0)}catch(i){throw console.error("Failed to load external dictionary:",i),i}}throw new Error("No dictionary available: model has no embedded dictionary and no fallback path provided")}}let Rt=null,Oe=null;self.addEventListener("message",async e=>{const{type:t,data:r}=e.data;switch(t){case"INIT":try{console.log("Initializing detection model:",r.modelPath),Rt=await Qh.loadModel(r.modelPath),console.log("Detection model initialized successfully"),console.log("Input names:",Rt.session.inputNames),console.log("Output names:",Rt.session.outputNames),Oe=r.config?.det||{engine_type:"onnxruntime",lang_type:"en",model_type:"mobile",ocr_version:"PP-OCRv4",limit_side_len:736,limit_type:"min",std:[.229,.224,.225],mean:[.485,.456,.406],scale:.00392156862745098,thresh:.3,box_thresh:.5,max_candidates:1e3,unclip_ratio:1.6,use_dilation:!0,score_mode:"fast"},self.postMessage({type:"RESULT",data:{initialized:!0}})}catch(i){self.postMessage({type:"ERROR",error:i.message})}break;case"DETECT":if(!Rt||!Oe){self.postMessage({type:"ERROR",error:"Model not initialized"});return}try{const{imageData:i,width:a,height:n}=r,{resizedData:s,resizedWidth:u,resizedHeight:l,ratioH:d,ratioW:c}=Yh(i,a,n),f=Jh(s,u,l),g={[Rt.session.inputNames[0]||"input"]:f},_=await Rt.session.run(g),b=em(_,u,l,d,c);self.postMessage({type:"RESULT",data:{boxes:b}})}catch(i){self.postMessage({type:"ERROR",error:i.message})}break}});function Yh(e,t,r){if(!Oe)throw new Error("Config not initialized");const i=Oe.limit_side_len,a=Oe.limit_type;let n=1;a==="max"?Math.max(r,t)>i&&(n=r>t?i/r:i/t):Math.min(r,t)<i&&(n=r<t?i/r:i/t);let s=Math.round(r*n),u=Math.round(t*n);s=Math.round(s/32)*32,u=Math.round(u/32)*32,s=Math.max(s,32),u=Math.max(u,32),console.log(`Detection resize: ${t}x${r} → ${u}x${s} (ratio: ${n.toFixed(3)})`);const d=new OffscreenCanvas(u,s).getContext("2d"),c=new ImageData(new Uint8ClampedArray(e),t,r),f=new OffscreenCanvas(t,r);return f.getContext("2d").putImageData(c,0,0),d.drawImage(f,0,0,t,r,0,0,u,s),{resizedData:d.getImageData(0,0,u,s).data,resizedWidth:u,resizedHeight:s,ratioH:s/r,ratioW:u/t}}function Jh(e,t,r){if(!Oe)throw new Error("Config not initialized");const i=3,a=new Float32Array(i*r*t),{mean:n,std:s,scale:u}=Oe;for(let l=0;l<r;l++)for(let d=0;d<t;d++){const c=(l*t+d)*4,f=l*t+d,h=e[c]*(u||1/255),g=e[c+1]*(u||1/255),_=e[c+2]*(u||1/255);a[f]=(h-n[0])/s[0],a[r*t+f]=(g-n[1])/s[1],a[2*r*t+f]=(_-n[2])/s[2]}return new Ue("float32",a,[1,i,r,t])}function em(e,t,r,i,a){if(!Oe)throw new Error("Config not initialized");const n=Object.keys(e)[0],s=e[n],u=s.data,l=s.dims;console.log("Detection output shape:",l);const[d,c,f,h]=l,g=new Uint8Array(f*h);for(let b=0;b<f;b++)for(let k=0;k<h;k++){const v=b*h+k,w=u[v];g[v]=w>Oe.thresh?255:0}return Oe.use_dilation&&tm(g,h,f),rm(g,h,f,t,r).map(b=>({topLeft:{x:Math.round(b.topLeft.x/a),y:Math.round(b.topLeft.y/i)},topRight:{x:Math.round(b.topRight.x/a),y:Math.round(b.topRight.y/i)},bottomRight:{x:Math.round(b.bottomRight.x/a),y:Math.round(b.bottomRight.y/i)},bottomLeft:{x:Math.round(b.bottomLeft.x/a),y:Math.round(b.bottomLeft.y/i)}}))}function tm(e,t,r){const i=new Uint8Array(e);for(let a=1;a<r-1;a++)for(let n=1;n<t-1;n++){const s=a*t+n;let u=!1;for(let l=-1;l<=1;l++){for(let d=-1;d<=1;d++){const c=(a+l)*t+(n+d);if(i[c]===255){u=!0;break}}if(u)break}e[s]=u?255:0}}function rm(e,t,r,i,a){if(!Oe)throw new Error("Config not initialized");const n=new Uint8Array(e.length),s=[];for(let u=0;u<r;u++)for(let l=0;l<t;l++){const d=u*t+l;if(e[d]===255&&!n[d]){const c=im(e,n,l,u,t,r);if(c.pixels>10){const f=i/t,h=a/r,g=(c.maxX-c.minX)*(Oe.unclip_ratio-1)/2,_=(c.maxY-c.minY)*(Oe.unclip_ratio-1)/2,b={topLeft:{x:Math.max(0,(c.minX-g)*f),y:Math.max(0,(c.minY-_)*h)},topRight:{x:Math.min(i,(c.maxX+g)*f),y:Math.max(0,(c.minY-_)*h)},bottomRight:{x:Math.min(i,(c.maxX+g)*f),y:Math.min(a,(c.maxY+_)*h)},bottomLeft:{x:Math.max(0,(c.minX-g)*f),y:Math.min(a,(c.maxY+_)*h)}};am(e,c,t)>Oe.box_thresh&&s.push(b)}}}return s}function im(e,t,r,i,a,n){const s={minX:r,maxX:r,minY:i,maxY:i,pixels:0},u=[[r,i]];for(;u.length>0;){const[l,d]=u.pop(),c=d*a+l;l<0||l>=a||d<0||d>=n||t[c]||e[c]!==255||(t[c]=1,s.pixels++,s.minX=Math.min(s.minX,l),s.maxX=Math.max(s.maxX,l),s.minY=Math.min(s.minY,d),s.maxY=Math.max(s.maxY,d),u.push([l+1,d],[l-1,d],[l,d+1],[l,d-1]))}return s}function am(e,t,r){let i=0,a=0;for(let n=t.minY;n<=t.maxY;n++)for(let s=t.minX;s<=t.maxX;s++)i+=e[n*r+s]/255,a++;return a>0?i/a:0}})();
