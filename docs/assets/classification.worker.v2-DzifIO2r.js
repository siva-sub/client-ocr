...
`).filter(n=>n.length>0)}catch(i){throw console.error("Failed to load external dictionary:",i),i}}throw new Error("No dictionary available: model has no embedded dictionary and no fallback path provided")}}let It=null,je=null;self.addEventListener("message",async e=>{const{type:t,data:r}=e.data;switch(t){case"INIT":try{console.log("Initializing classification model:",r.modelPath),It=await Xh.loadModel(r.modelPath),console.log("Classification model initialized successfully"),console.log("Input names:",It.session.inputNames),console.log("Output names:",It.session.outputNames),je=r.config?.cls||{engine_type:"onnxruntime",lang_type:"en",model_type:"mobile",ocr_version:"PP-OCRv4",cls_image_shape:[3,48,192],cls_batch_num:6,cls_thresh:.9,label_list:["0","180"]},self.postMessage({type:"RESULT",data:{initialized:!0}})}catch(i){self.postMessage({type:"ERROR",error:i.message})}break;case"CLASSIFY":if(!It||!je){self.postMessage({type:"ERROR",error:"Model not initialized"});return}try{const{images:i}=r,a=[],n=je.cls_batch_num||6;for(let s=0;s<i.length;s+=n){const u=i.slice(s,s+n),l=await Yh(u);a.push(...l),self.postMessage({type:"PROGRESS",progress:Math.min((s+n)/i.length,1)})}self.postMessage({type:"RESULT",data:{results:a}})}catch(i){self.postMessage({type:"ERROR",error:i.message})}break}});async function Yh(e){if(!It||!je)throw new Error("Model not initialized");const t=[];for(const _ of e){const b=Jh(_.data,_.width,_.height);t.push(b)}const r=new Float32Array(t.length*t[0].data.length);t.forEach((_,b)=>{r.set(_.data,b*_.data.length)});const[i,a,n,s]=[t.length,...je.cls_image_shape],u=new Me("float32",r,[i,a,n,s]),d={[It.session.inputNames[0]||"input"]:u},c=await It.session.run(d),f=[],g=c[Object.keys(c)[0]].data;for(let _=0;_<i;_++){const b=em(g,_);f.push(b)}return f}function Jh(e,t,r){if(!je)throw new Error("Config not initialized");const[i,a,n]=je.cls_image_shape,u=new OffscreenCanvas(n,a).getContext("2d"),l=new ImageData(new Uint8ClampedArray(e),t,r),d=new OffscreenCanvas(t,r);d.getContext("2d").putImageData(l,0,0),u.drawImage(d,0,0,t,r,0,0,n,a);const f=u.getImageData(0,0,n,a).data,m=new Float32Array(i*a*n);for(let g=0;g<a;g++)for(let _=0;_<n;_++){const b=(g*n+_)*4,k=g*n+_,v=f[b]/255,w=f[b+1]/255,S=f[b+2]/255;m[k]=(v-.5)/.5,m[a*n+k]=(w-.5)/.5,m[2*a*n+k]=(S-.5)/.5}return new Me("float32",m,[1,i,a,n])}function em(e,t){if(!je)throw new Error("Config not initialized");const r=je.label_list.length,i=t*r,a=e.slice(i,i+r),n=Math.max(...a),s=a.map(g=>Math.exp(g-n)),u=s.reduce((g,_)=>g+_,0),l=s.map(g=>g/u);let d=0,c=l[0];for(let g=1;g<l.length;g++)l[g]>c&&(c=l[g],d=g);const f=je.label_list[d]==="180"?180:0,m=f===180&&c>=je.cls_thresh;return{angle:f,confidence:c,shouldRotate:m}}})();
